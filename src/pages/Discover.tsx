...
import { Search, UserPlus, Clock, MessageSquare, Check } from "lucide-react";
      ...
      <main className="flex-1 pb-20">
        <div className="mx-auto max-w-lg px-4 py-4 space-y-4">
          ...
          {loading ? (
            ...
            <p className="text-center text-sm text-muted-foreground py-16">Aucun utilisateur trouvé</p>
          ) : (
            <div className="grid grid-cols-2 gap-3">
              {users.map((u, i) => {
                ...
                return (
                  <Card
                    key={u.user_id}
                    className="flex flex-col items-center gap-2.5 p-4 hover:shadow-lg transition-all duration-200 animate-scale-in border-border/50"
                    style={{ animationDelay: `${i * 30}ms` }}
                  >
                    <Avatar className="h-18 w-18 ring-2 ring-accent">
                    ...
                    <div className="flex gap-1.5 w-full">
                      {status === "none" && (
                        <Button size="sm" className="flex-1 text-xs gradient-primary border-0 rounded-full" onClick={() => sendRequest(u.user_id)}>
                          <UserPlus className="mr-1 h-3.5 w-3.5" /> Ajouter
                        </Button>
                      )}
                      {status === "sent" && (
                        <Button size="sm" variant="secondary" className="flex-1 text-xs rounded-full" disabled>
                          <Clock className="mr-1 h-3.5 w-3.5" /> Envoyée
                        </Button>
                      )}
                      {status === "friend" && (
                        <Button size="sm" variant="secondary" className="flex-1 text-xs rounded-full" onClick={() => handleMessage(u.user_id)}>
